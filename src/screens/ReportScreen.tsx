...
export default function ReportScreen() {
  ...
  const sugars100 = product?.nutrients?.sugars100 ?? nutriments['sugars_100g'] ?? nutriments?.sugars100g ?? 0;
  const fat100 = product?.nutrients?.fat100 ?? nutriments['fat_100g'] ?? nutriments?.totalFat100g ?? 0;
  const satFat100 = product?.nutrients?.satFat100 ?? nutriments['saturated-fat_100g'] ?? nutriments?.saturatedFat100g ?? 0;
  const sodium100 = product?.nutrients?.sodium100 ?? nutriments['salt_100g'] ?? nutriments?.salt100g ?? 0;

  // Semáforo
  const levels = product?.nutrient_levels || {};
  const sugarLevel = (product?.trafficLight?.sugar || levels['sugars'] || nutriments?.trafficLightSugar || 'BAJO').toUpperCase();
  const fatLevel = (product?.trafficLight?.satFat || levels['saturated-fat'] || nutriments?.trafficLightSaturatedFat || 'BAJO').toUpperCase();
  const sodiumLevel = (product?.trafficLight?.sodium || levels['salt'] || nutriments?.trafficLightSodium || 'BAJO').toUpperCase();
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.semaphoreContainer}>
        <View style={styles.nutrientCard}>
          ...
          <Text style={styles.gramText}>{sugars100}g / 100{quantityUnit}</Text>
        ...
        <View style={styles.nutrientCard}>
          ...
          <Text style={styles.gramText}>{satFat100}g / 100{quantityUnit}</Text>
        ...
        <View style={styles.nutrientCard}>
          ...
          <Text style={styles.gramText}>{sodium100}g / 100{quantityUnit}</Text>
      ...
      <View style={styles.educationalCard}>
        ...
        <Text style={styles.educationalText}>
          Por cada <Text style={{ fontWeight: '700' }}>{portionLabel}</Text>, este producto contiene aprox.{' '}
        ...
        <View style={styles.spoonRow}>
          <Text style={styles.spoonText}>
            Equivale a: {teaspoons > 0 ? '🥄 '.repeat(Math.min(teaspoons, 8)) : '0 cucharaditas'} ({teaspoons} tsp)
          </Text>
        </View>
      </View>
      ...
      >
        <View style={styles.expandableHeader}>
          <Text style={styles.expandableTitle}>🧈 Grasas Totales ({fat100} g / 100{quantityUnit})</Text>
          <Text style={styles.arrow}>{showFatDetails ? '▲' : '▼'}</Text>
        </View>

        {showFatDetails && (
          <View style={styles.expandableContent}>
            <Text style={styles.fatItem}>🟢 <Text style={styles.bold}>Grasas monoinsaturadas:</Text> Principalmente ácido oleico.</Text>
            <Text style={styles.fatItem}>🟢 <Text style={styles.bold}>Grasas poliinsaturadas:</Text> Omega-6 y pequeñas cantidades de Omega-3.</Text>
            <Text style={styles.fatItem}>🟠 <Text style={styles.bold}>Grasas saturadas ({satFat100} g):</Text> Su consumo excesivo eleva el colesterol LDL ("malo") y aumenta el riesgo cardiovascular.</Text>
            <Text style={styles.fatItem}>⚪ <Text style={styles.bold}>Grasas trans:</Text> Grasas dañinas que elevan el colesterol malo y reducen el bueno, elevando el riesgo cardiovascular.</Text>
          </View>
        )}
      </TouchableOpacity>
      ...
      >
        ...
        {showSaltDetails && (
          <View style={styles.expandableContent}>
            ...
            <Text style={styles.bulletPoint}>• Un alto consumo de sal (o sodio) puede provocar un aumento de la presión arterial y el riesgo de enfermedades cardiovasculares.</Text>
            <Text style={styles.bulletPoint}>• Muchas personas con presión alta no lo saben, ya que no presenta síntomas previos.</Text>
            <Text style={styles.bulletPoint}>• La mayoría de las personas consumen de 9 a 12 g diarios de sal, el doble del límite recomendado (5 g/día según la OMS).</Text>
...
const styles = StyleSheet.create({
  ...
  headerCard: { width: '100%', backgroundColor: '#FFFFFF', borderRadius: 16, padding: 16, marginBottom: 16, elevation: 2 },
  ...
  sectionTitle: { fontSize: 17, fontWeight: '600', color: '#1C1C1E', alignSelf: 'flex-start', marginBottom: 10 },
  semaphoreContainer: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', marginBottom: 16 },
  nutrientCard: { width: (width - 56) / 3, backgroundColor: '#FFFFFF', borderRadius: 12, padding: 10, alignItems: 'center', elevation: 1 },
  nutrientLabel: { fontSize: 12, fontWeight: '600', color: '#3A3A3C' },
  gramText: { fontSize: 11, color: '#8E8E93', marginVertical: 4 },
  badge: { paddingVertical: 4, paddingHorizontal: 8, borderRadius: 12, width: '100%', alignItems: 'center' },
  badgeText: { fontSize: 11, fontWeight: '700' },
  educationalCard: { width: '100%', backgroundColor: '#FFF4E5', borderRadius: 12, padding: 14, borderLeftWidth: 4, borderLeftColor: '#FF9500', marginBottom: 14 },
  ...
  expandableCard: { width: '100%', backgroundColor: '#FFFFFF', borderRadius: 12, padding: 14, marginBottom: 12, elevation: 1 },
  ...
  expandableContent: { marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#E5E5EA' },
  ...
  backButton: { backgroundColor: '#000000', paddingVertical: 14, borderRadius: 30, width: '100%', alignItems: 'center', marginTop: 10, marginBottom: 20 },
